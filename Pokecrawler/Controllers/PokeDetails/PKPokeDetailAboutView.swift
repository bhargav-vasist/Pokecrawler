...
class PKPokeDetailAboutView: UIView {
    ...
    lazy private var typeStack: PKPokemonTypeStackView = {
        let stack = PKPokemonTypeStackView(with: pokemonModel.types)
        addSubview(stack)
        return stack
    }()
    ...
    lazy var pokeBodyStatView: PKPokeDetailCardView = {
        let viewModel = PKPokeDetailCardViewModel(with: pokemonModel)
        let bsView = PKPokeDetailCardView(with: viewModel)
    ...
    func loadView(with pokeSpecies: PKPokemonSpecies) {
        ...
        DispatchQueue.main.async {
            ...
            self.pokeDescriptionLabel.text = self.pokemonSpecies?.flavorTextEntries?.first(where: {
                $0.language?.name == "en"
            })?
                .flavorText?
